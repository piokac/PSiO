...
{
public:
    TicTacToeAbstract() = default;
    virtual std::pair<Player, Status> move(const Position &pos, Player player = Player::null) = 0;
    virtual Player active_player() const = 0;
    virtual const std::vector<std::vector<Player>> &state() = 0;
    virtual Status is_finished() const = 0;
    virtual void restart() = 0;
};

class TicTacToe : public TicTacToeAbstract
{
protected:
    size_t size_;
    Board board_;
...
public:
    ...
    ///
    const std::vector<std::vector<Player>> &state() { return board_; }
