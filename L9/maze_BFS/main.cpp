...
public:
    ...
    bool is_exit(size_t r, size_t c) const
    {
    ...
    std::vector<std::pair<size_t, size_t>> find_exits(size_t pos_row, size_t pos_col)
    {
        ...
        q.emplace(pos_row, pos_col); //dadaje węzeł (parę współrzędnych) do kolejki
        do {
            ...
            if (!maze[row][col].visited) {
                ...
                std::vector<std::pair<size_t, size_t>> nodes = neighbours(row, col);
                for (auto el : nodes) { //dodaje wszystkie nieodwiedzone węzły sąsiadów do kolejki
                    q.emplace(el);
                }
            }
            q.pop(); //usuwa odwiedzony węzeł z kolejki
